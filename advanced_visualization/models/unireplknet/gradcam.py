...
class UniRepLKNetGradcamEngine:
    ...
    def __init__(self) -> None:
        ...
        self._bundles: dict[str, GradcamBundle] = {}
        ...
        with self._lock:
            ...
            if not config.checkpoint.exists():
                raise FileNotFoundError(
                    f"Grad-CAM checkpoint missing: {config.checkpoint}"
                )
    ...
    def score(self, model: torch.nn.Module, input_tensor: torch.Tensor) -> torch.Tensor:
        ...
        head = getattr(model, "mlp_head")
        features = feature_extractor(input_tensor)
        head_sequence = getattr(head, "fc", None)
        if head_sequence is None:
            head_sequence = getattr(head, "head", None)
    ...
    def compute_cam(
        ...
        if method in {"gradcam++", "gradcampp"}:
            ...
            denominator = 2.0 * gradients_power_2 + (
                activation * gradients_power_3
    ...
    def generate(
        ...
        output_path = gradcam_cache_candidates(output_root, image_path, target=target)[
            0
        ]
        ...
        try:
            ...
            jet_overlay(
                np.asarray(image.convert("RGB")), cam.detach().cpu().numpy()
            ).save(tmp_output, format="PNG", compress_level=3)
    ...
    def _load_state_dict(
        self, model: torch.nn.Module, checkpoint_path: Path, device: torch.device
    ) -> None:
