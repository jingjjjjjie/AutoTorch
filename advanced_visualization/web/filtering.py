...
def _known_column(frame: pd.DataFrame, value: str) -> str | None:
    return (
        value
        if value and value in frame.columns and value not in INTERNAL_COLUMNS
        else None
    )
...
def _apply_failure_filter(frame: pd.DataFrame, request: FilterRequest) -> pd.DataFrame:
    ...
    if mode == "high_confidence":
        return frame[
            frame["__is_failure"] & frame["__confidence"].ge(request.high_confidence)
        ]
    if mode == "low_confidence":
        return frame[
            frame["__is_failure"] & frame["__confidence"].le(request.low_confidence)
        ]
    return frame
...
def filter_frame(frame: pd.DataFrame, request: FilterRequest) -> pd.DataFrame:
    ...
    search_columns = [
        column for column in request.search_columns if _known_column(result, column)
    ]
    result = apply_text_search(result, request.search, search_columns)
    categorical_filters = {
        column: values
        for column, values in request.categorical_filters.items()
        if _known_column(result, column)
    }
    result = apply_categorical_filters(result, categorical_filters)
...
def page_frame(
    frame: pd.DataFrame, request: FilterRequest
    ...
    metrics = {
        ...
        "high_confidence_failures": int(
            (
                frame["__is_failure"]
                & frame["__confidence"].ge(request.high_confidence)
            ).sum()
        ),
