...
def _column_from_template(
    template: str, branch: dict, layer: dict, config: dict
) -> str:
...
def gradcam_column(branch: dict, layer: dict, config: dict) -> str:
    ...
    template = str(
        layer.get("column_template")
        or config.get("column_template")
        or "{branch}_{layer}_gradcam_path"
    )
...
def gradcam_candidates(branch: dict, layer: dict, config: dict) -> list[str]:
    candidates = [
        str(column)
        for column in layer.get("column_candidates", [])
        if str(column).strip()
    ]
...
def first_existing_gradcam_column(
    df: pd.DataFrame, branch: dict, layer: dict, config: dict
) -> str:
...
def _branch_image_default(columns: list[str], branch: dict) -> str:
    return first_existing(columns, branch.get("image_candidates", [])) or ""
...
def _source_prediction_column(source: dict, columns: list[str]) -> str:
    ...
    for configured_model in load_settings().models:
        if (
            configured_model.key == model_key
            and configured_model.prediction_column in columns
        ):
...
def _has_prepared_gradcam_fallback(source: dict, layer: dict, config: dict) -> bool:
    ...
    for root in _prepared_gradcam_roots(source):
        if root.exists() and _cached_gradcam_index(
...
def resolved_gradcam_path(
    row: pd.Series, controls: dict, branch: dict
) -> Path | str | None:
    gradcam_column = first_existing_gradcam_column(
        pd.DataFrame([row]), branch, controls["layer"], controls["config"]
    )
...
def sidebar_controls(df: pd.DataFrame, source: dict, config: dict) -> dict:
    ...
    subclass_default = first_existing(columns, SUBCLASS_COLUMNS) or first_existing(
        columns, config.get("metadata_columns", [])
    )
    truth_default = "label" if "label" in columns else subclass_default
    pred_default = _source_prediction_column(source, columns) or first_existing(
        columns, config.get("prediction_candidates", [])
    )
    ...
    default_layer_label = next(
        (
            _layer_label(layer)
            for layer in layers
            if layer.get("key") == default_layer_key
        ),
        _layer_label(layers[0]) if layers else "",
    )
    ...
    with st.sidebar.form(f"{config.get('model_type', 'extra')}_layered_gradcam_form"):
        submitted = st.form_submit_button(
            "Update workspace", type="primary", use_container_width=True
        )
        ...
        target = st.radio(
            "Target",
            target_options,
            index=len(target_options) - 1 if "All branches" in target_options else 0,
        )
        layer_label = st.radio(
            "Layer",
            layer_labels,
            index=_option_index(layer_labels, default_layer_label),
        )
        only_prepared = st.checkbox("Only rows with selected artifact", value=True)

        st.header("Columns")
        item_id_column = st.selectbox(
            "Item ID", columns, index=columns.index(id_default)
        )
        ...
        prediction_column = st.selectbox(
            "Prediction score",
            ["None"] + columns,
            index=(columns.index(pred_default) + 1 if pred_default in columns else 0),
        )
        ...
        prediction_threshold = st.slider(
            ...
            0.0,
            1.0,
            float(review.get("prediction_threshold", 0.5)),
            0.01,
        )
        truth_row_options = [
            "Valid only: 0 and 1",
        ...
        truth_rows = st.radio(
            ...
            index=_option_index(
                truth_row_options,
                review.get("default_truth_rows", "Valid only: 0 and 1"),
            ),
        )
        ...
        failure_view = st.radio(
            "Failure view",
            failure_view_options,
            index=_option_index(
                failure_view_options, review.get("default_failure_view", "All rows")
            ),
        )
        high_conf = st.slider(
            "High confidence >=",
            0.0,
            1.0,
            float(review.get("high_confidence", 0.9)),
            0.01,
        )
        low_conf = st.slider(
            "Low confidence <=",
            0.0,
            1.0,
            float(review.get("low_confidence", 0.6)),
            0.01,
        )
        ...
        search_columns = st.multiselect(
            ...
            default=[
                column
                for column in (item_id_column, subclass_column)
                if column and column != "None"
            ],
        )
        text_query = st.text_input("Search text")
        default_filter_columns = [
            column for column in config.get("metadata_columns", []) if column in cats
        ]
        filter_columns = st.multiselect(
            "Categorical filters", cats, default=default_filter_columns
        )
        categorical_filters = {}
        ...
        browse_mode = st.radio(
            "Browse mode", ["Bottomless scroll", "Pages"], index=0, horizontal=True
        )
        page_size = st.select_slider(
            "Page size",
            options=[12, 24, 48, 96],
            value=min(48, int(review.get("page_size", 48))),
        )
    ...
    selected_layer = next(
        layer for layer in layers if _layer_label(layer) == layer_label
    )
    return {
        "source": source,
        "config": config,
...
def apply_filters(df: pd.DataFrame, controls: dict) -> pd.DataFrame:
    ...
    filtered = apply_failure_mode(
        filtered, controls["failure_view"], controls["high_conf"], controls["low_conf"]
    )
    ...
    if controls["only_prepared"]:
        ...
        for branch in controls["branches"]:
            column = first_existing_gradcam_column(
                filtered, branch, controls["layer"], controls["config"]
            )
            if column in filtered.columns:
                mask |= filtered[column].map(_path_exists)
            else:
                mask |= filtered.apply(
                    lambda row: resolved_gradcam_path(row, controls, branch)
                    is not None,
                    axis=1,
                )
...
def render_summary(
    source: pd.DataFrame, filtered: pd.DataFrame, controls: dict
) -> None:
...
def _metadata_line(row: pd.Series, controls: dict) -> str:
    fields = []
    for column in [
        controls["subclass_column"],
        *controls["config"].get("metadata_columns", []),
    ]:
...
def _render_branch(
    ...
    *,
    ...
    image_column = controls["image_columns"].get(branch["key"])
    original_path = (
        row[image_column] if image_column and image_column in row.index else None
    )
...
def render_card(row: pd.Series, controls: dict, display_index: int) -> None:
    ...
    if controls.get("show_card_metadata", False):
        ...
        st.markdown(
            f'<div class="viewer-caption compact">{item}</div>', unsafe_allow_html=True
        )
...
def render_grid(df: pd.DataFrame, controls: dict, start_index: int = 1) -> None:
    ...
    for offset in range(0, len(rows), controls["columns_per_row"]):
        ...
        for column, (display_index, (_index, row)) in zip(
            columns, rows[offset : offset + controls["columns_per_row"]]
        ):
            with column:
...
def render(df: pd.DataFrame, source: dict, config: dict) -> None:
    ...
    description = html.escape(
        str(
            ...
            or "Model-specific layered Grad-CAM artifact review."
        )
    )
    st.markdown(
        f"""
    ...
    missing = [
        ...
        for branch in controls["branches"]
        if first_existing_gradcam_column(df, branch, controls["layer"], config)
        not in df.columns
