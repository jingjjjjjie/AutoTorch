"""
OverLoCK Training Script
Run with: CUDA_VISIBLE_DEVICES=0,1 torchrun --nproc_per_node=2 train_overlock.py
Test with: CUDA_VISIBLE_DEVICES=1 torchrun --nproc_per_node=1 train_overlock.py
"""
import sys
...
from training.trainers import idfraud_trainer_overlock
...
def main():
    ...
    # build_model loads the OverLoCK backbone via load_overlock_model (pretrained ImageNet-1K
    # weights, head[-1] replaced with Identity) then attaches the idfraud classification head
    model = build_model(
        model_name=cfg.model.backbone_name,
        device=local_rank,
        task=cfg.model.get('task', 'overlock_classification'),
        head_type=cfg.model.head_type,
        freeze_backbone=cfg.model.freeze_backbone)
    ddp_model = wrap_model_ddp(model, local_rank)

    loss_fn = build_loss_fn(cfg.training.loss_fn)
    optimizer = build_optimizer(name=cfg.training.optimizer, model=ddp_model,
    ...
    # use OverLoCK-specific trainer that handles deep-supervision loss during training
    results = idfraud_trainer_overlock.train(
        model=ddp_model,
    ...
    save_after_training(cfg.run_dir, ddp_model=ddp_model,
                        save_name=cfg.experiment.save_name, timer=timer)
