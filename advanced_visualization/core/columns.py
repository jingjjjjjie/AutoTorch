...
from advanced_visualization.core.config import ID_COLUMNS, IMAGE_COLUMNS, SUBCLASS_COLUMNS
...
def infer_standard_columns(df: pd.DataFrame, prediction_column: str = "") -> dict[str, str]:
    ...
    return {
        ...
        "image_column": first_existing(columns, IMAGE_COLUMNS) or (image_columns[0] if image_columns else ""),
        "subclass_column": subclass_default or "",
        "truth_column": "label" if "label" in df.columns else (subclass_default or ""),
        "prediction_column": prediction_column if prediction_column in df.columns else (prediction_candidates[-1] if prediction_candidates else ""),
    }
