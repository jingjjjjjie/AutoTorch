...
def analysis_page(repository: DatasetRepository, request: AnalysisRequest) -> dict:
    ...
    if request.item_id_column not in frame:
        raise ValueError(f"Analysis is missing item ID column: {request.item_id_column}")
    ...
    if request.search.strip():
        selected &= result.frame[request.item_id_column].astype(str).str.contains(
    ...
    working["__merged_confidence"] = np.maximum(working["__merged_score"], 1.0 - working["__merged_score"])
    ...
    for _, row in page.iterrows():
        ...
        rows.append({
            ...
            "subclass": json_value(row.get(request.subclass_column)) if request.subclass_column else None,
            "identity": json_value(row.get(request.identity_column)) if request.identity_column else None,
            "quality": json_value(row.get(request.quality_column)) if request.quality_column else None,
            "original_image_url": image_url(source.id, row_id, request.original_image_column),
            "crop_image_url": image_url(source.id, row_id, request.crop_image_column),
        })
    breakdowns = {
        "subclass": grouped_analysis(result, (request.subclass_column,)),
        "identity": grouped_analysis(result, (request.identity_column,)),
        "identity_subclass": grouped_analysis(result, (request.identity_column, request.subclass_column)),
    }
    return {
        "total": total,
